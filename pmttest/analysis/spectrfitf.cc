...
Double_t pmtfit(Double_t* x, Double_t* par) {
  ...
  =============================================================*/
  int maxN=6, minN=0; //min and max number of p.e.
...
Double_t pmtfit_bessel(Double_t* x, Double_t* par) {
  /*===========================================================
         par[0]  ->   Poisson average N
         par[1]  ->   1 p.e. ADC separation (gain)
	 par[2]  ->   dummy unused, allows to keep same structure as pmtfit function
         par[3]  ->   N=0 ADC offset
         par[4]  ->   sigma noise fabs(vecxp.at(1)-vecxp.at(0))
         par[5]  ->   Absolute count normalisation
	 par[6]  ->   Fraction of gain on first dynode
  ...
  for (int i=0; i<maxN; i++) {
    if (i==0) {
      ...
      ci = ci*mean_npe/((float)i);
      ci2 = ci2*mean_npe1/((float)i);
    }
  ...
  Double_t xm=*x-ped;
  Double_t NDynodes=12.;
  Double_t ADCoverEle=50./1e6;//estimate of conversion from ADC to electrons from circuit simulation
  Double_t AoverB=2*pow(gain/ADCoverEle,1./NDynodes);
  Double_t sqrtAXoverBsq=AoverB*sqrt(xm/gain);
  Double_t AoverB1=2*pow(gain1/ADCoverEle,1./(NDynodes-1));
  Double_t sqrtAXoverB1sq=AoverB1*sqrt(xm/gain1);
  Double_t valK=0;
  Double_t val1=0;
  ...
  for (int i=minN; i<maxN; i++) {
    ...
    //    if (debug)     printf (" sigma = %f, noise = %f, xm = %f, grms = %f\n",sigma,noise,xm,grms);
    if (i==0) { //no signal, only dark noise
      valK += probarr[i] * exp(-0.5*(xm*xm)/noise/noise)*gausfac/noise ;
      val1 += probarr1[i] * exp(-0.5*(xm*xm)/noise/noise)*gausfac/noise ;	 
    } else {
      if (*x - ped >=1e-10){
	//note: normalization seems wrong....
	valK += probarr[i] *  exp(-AoverB*( 1+xm/gain ))*sqrtAXoverBsq *TMath::BesselI1(2*sqrtAXoverBsq);
	val1 += probarr1[i] * exp(-AoverB1*(1+xm/gain1))*sqrtAXoverB1sq*TMath::BesselI1(2*sqrtAXoverB1sq);
      }
    }
  }
  return   norm*eff*((1-frac)*valK+frac*val1)*bw;
