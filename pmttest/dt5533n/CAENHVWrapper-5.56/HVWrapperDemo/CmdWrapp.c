...
void HVSystemLogin()
{
	...
	con_printf("( SY1527 -> 0,  SY2527 -> 1, SY4527 -> 2, SY5527 -> 3, N568 -> 4, V65XX -> 5, N147X -> 6 ): ");
	c = con_getch();
	while( c != '0' && c != '1' && c != '2' && c != '3' && c != '4' && c != '5' && c != '6' )
		c = con_getch();
	sysType= c - '0';

	con_printf("\nComm. Layer: (""TCP/IP""-> 0, ""RS232""-> 1, ""CAENET""->2, ""USB""->3, ""OPTICAL LINK""->4  ): ");
	c = con_getch();
	...
	ret = CAENHV_InitSystem(sysType, link, arg, userName, passwd, &sysHndl);
...
do{
	ret = CAENHV_TestBdPresence(handle, slot, &NrOfCh, &Model, &Descr, &serNumb, &fmwMin, &fmwMax);
	if( ret != CAENHV_OK )
	  {
...
/* NrWCode */
	...
	ret = CAENHV_CaenetComm(handle,crate,code,NrWCode,WCode,&ErrorCodeCaenet,&NrOfData,&Data);
	if( ret != CAENHV_OK )
		con_printf("CAENHV_CaenetComm: %s (num. %d)\n\n", CAENHV_GetError(handle), ret);
	else
	{
		clrscr();
		gotoxy(1,2);
