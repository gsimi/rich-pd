...
// myRooGamma(_x=mu,_gamma=N+1,_beta=1,_mu=0)
...
myRooGamma::myRooGamma(const char *name, const char *title,
...
//_____________________________________________________________________________
myRooGamma::myRooGamma(const myRooGamma& other, const char* name) : 
...
//_____________________________________________________________________________
Double_t myRooGamma::evaluate() const
{
...
//_____________________________________________________________________________
Int_t myRooGamma::getAnalyticalIntegral(RooArgSet& allVars, RooArgSet& analVars, const char* /*rangeName*/) const 
{
...
//_____________________________________________________________________________
Double_t myRooGamma::analyticalIntegral(Int_t code, const char* rangeName) const 
{
  assert(code==1) ;

 //integral of the Gamma distribution via ROOT::Math
  double xmin=x.min(rangeName);
  if (x.min(rangeName)<mu) xmin=mu;
  Double_t integral = ROOT::Math::gamma_cdf(x.max(rangeName), gamma, beta, mu) - ROOT::Math::gamma_cdf(xmin, gamma, beta, mu);
  return integral ;
...
//_____________________________________________________________________________
Int_t myRooGamma::getGenerator(const RooArgSet& directVars, RooArgSet &generateVars, Bool_t /*staticInitOK*/) const
{
...
//_____________________________________________________________________________
void myRooGamma::generateEvent(Int_t code)
{
  assert(code==1) ;
