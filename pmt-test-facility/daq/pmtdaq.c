...
Restart:
    ...
    /* *************************************************************************************** */
    while(!WDrun.Quit) {
      if (WDcfg.EventsToWrite > 0 && WDrun.AcqRun == 0){ // non interactive mode 
        CAEN_DGTZ_SWStartAcquisition(handle);
	WDrun.AcqRun = 1;
	WDrun.ContinuousWrite = 1;
	printf("starting non interactive data acquisition of %d events\n",WDcfg.EventsToWrite);
      }
